...
tf = 'test_TestcaseNo100032'

#Test case No : 100032
class TestcaseNo100032(unittest.TestCase):
    def test_TestcaseNo100032(self) :
        try:
            ...
            password.send_keys(registrationform_testdata['password_withoutspecialcharacter'])
            ...
            self.assertEqual(validationmessage_invalidinput.text,'Password should Contain atleast one uppercase letter,1 number and one special character')

        except Exception:
            traceback.print_exc()
            browser.save_screenshot(folder_path+'\Screenshots\Testcase-%s.png' %tf)
            self.fail("Test case No : 100032 is failed")
        finally:
